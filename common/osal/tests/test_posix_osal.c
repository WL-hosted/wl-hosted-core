...
static void wait_callback(void *argument) {
    callback_context_t *context = argument;
    assert(
        context->ops->semaphore_take(
            context->ops->context, context->semaphore, WLH_OSAL_WAIT_FOREVER
        ) == 0
    );
}
...
int main(void) {
    ...
    before_sleep = ops.monotonic_time_ms(ops.context);
    ops.sleep_ms(ops.context, 2u);
    assert(ops.monotonic_time_ms(ops.context) >= before_sleep);
    ...
    assert(
        ops.queue_receive(ops.context, &queue, &received, WLH_OSAL_NO_WAIT) != 0
    );
    assert(
    ...
    assert(
        ops.task_create(ops.context, &task, NULL, signal_callback, &callback) ==
        0
    );
    ...
    assert(
        ops.task_create(ops.context, &task, NULL, wait_callback, &callback) == 0
    );
    ...
    assert(
        ops.timer_create(ops.context, &timer, signal_callback, &callback) == 0
    );
